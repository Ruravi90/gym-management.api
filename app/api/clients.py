...
def search_clients(search: str, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
...
async def register_face(client_id: int, file: UploadFile = File(...), db: Session = Depends(get_db)):
...
def get_face_registration_status(client_id: int, db: Session = Depends(get_db)):
...
def remove_face_registration(client_id: int, db: Session = Depends(get_db)):
