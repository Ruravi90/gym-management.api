...
async def get_measurements(
    ...
    """Medidas del cliente autenticado, con delta vs el registro anterior."""
    client = await get_current_client(current_user)
    return await crud.measurement.list_measurements(client_id=client.id, limit=limit)
...
async def create_measurement(
    ...
    client = await get_current_client(current_user)
    return await crud.measurement.upsert_measurement(client_id=client.id, data=measurement)
...
async def update_measurement(
    ...
    if not db_measurement or db_measurement.client_id != client.id:
